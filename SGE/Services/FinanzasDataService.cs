...
public interface IFinanzasDataService
{
    ...
    void ExecuteNonQuery(string sql, Action<NpgsqlParameterCollection> parameters);
...
values (@numero, CURRENT_DATE, @libro, @glosa, @documento)
returning asientoid", cn, tx))
        {
    ...
    public void ActualizarMovimientoTesoreria(long movimientoTesoreriaId, int cuentaBancariaId, string tipoFlujo, string medioPagoSunat, decimal monto, string glosaMovimiento)
    {
        ...
        var oldFlujo = "ing";
        var oldMonto = 0m;
        using (var read = new NpgsqlCommand("select cuentabancariaid, tipoflujo, monto from finanzas.movimientostesoreria where movimientotesoreriaid = @id", cn, tx))
        {
...
where movimientotesoreriaid = @id", cn, tx))
    ...
    public void ExecuteNonQuery(string sql, Action<NpgsqlParameterCollection> parameters)
    {
        using var cn = CrearConexion();
    ...
    private static List<T> Query<T>(NpgsqlConnection cn, string sql, Func<NpgsqlDataReader, T> map)
    {
        using var cmd = new NpgsqlCommand(sql, cn);
    ...
    private static void InsertDetalle(NpgsqlConnection cn, NpgsqlTransaction tx, long asientoId, string cuentaCodigo, decimal debe, decimal haber)
    {
        using var cmd = new NpgsqlCommand("insert into finanzas.asientosdetalle (asientoid, cuentacodigo, debe, haber) values (@asiento, @cuenta, @debe, @haber)", cn, tx);
    ...
    private static void AjustarSaldoCuenta(NpgsqlConnection cn, NpgsqlTransaction tx, int cuentaBancariaId, decimal ajuste)
    {
        using var cmd = new NpgsqlCommand("update finanzas.cuentasbancarias set saldoactual = saldoactual + @ajuste where cuentabancariaid = @cuenta", cn, tx);
...
public static class FinanzasFormat
{
    ...
    {
        return amount.HasValue ? "S/ " + amount.Value.ToString("#,##0.00", CultureInfo.InvariantCulture) : "S/ 0.00";
    }
