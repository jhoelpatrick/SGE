...
create table comercial.clientes (
    clienteid int GENERATED ALWAYS AS IDENTITY constraint pk_comercial_clientes primary key,
...
create table comercial.contactosclientes (
    contactoclienteid int GENERATED ALWAYS AS IDENTITY constraint pk_comercial_contactosclientes primary key,
...
create table comercial.proveedores (
    proveedorid int GENERATED ALWAYS AS IDENTITY constraint pk_comercial_proveedores primary key,
    tipodocumento char(1) not null, 
    numerodocumento varchar(15) not null,
    razonsocial varchar(250) not null,
    direccionfiscal varchar(500) null,
...
create table comercial.contactosproveedores (
    contactoproveedorid int GENERATED ALWAYS AS IDENTITY constraint pk_comercial_contactosproveedores primary key,
...
create table comercial.vehiculosproveedores (
    vehiculoid int GENERATED ALWAYS AS IDENTITY constraint pk_comercial_vehiculosproveedores primary key,
...
create table comercial.conductoresproveedores (
    conductorid int GENERATED ALWAYS AS IDENTITY constraint pk_comercial_conductoresproveedores primary key,
...
create table comercial.productos (
    productoid int GENERATED ALWAYS AS IDENTITY constraint pk_comercial_productos primary key,
    ...
    nosevende BOOLEAN DEFAULT FALSE,           
    sefabrica BOOLEAN DEFAULT FALSE,           
    estado BOOLEAN DEFAULT TRUE
);
...
-- ==========================================================================================
CREATE INDEX ix_clientes_busqueda on comercial.clientes (numerodocumento, tipodocumento) include (razonsocial);
CREATE INDEX ix_vehiculos_placabusqueda on comercial.vehiculosproveedores (placa) include (proveedorid, estado);
...
CREATE OR REPLACE VIEW comercial.vw_crm_clientes_bandeja as
select 
    c.clienteid,
    c.tipodocumento,
...
CREATE OR REPLACE VIEW comercial.vw_logistica_checkinflotas as
select 
    v.placa,
    v.marca,
    v.modelo,
...
left join comercial.conductoresproveedores cond on p.proveedorid = cond.proveedorid and cond.estado = TRUE;
...
create table operaciones.proyectos (
    proyectoid int GENERATED ALWAYS AS IDENTITY constraint pk_operaciones_proyectos primary key,
...
create table operaciones.proyectotareas (
    tareaid int GENERATED ALWAYS AS IDENTITY constraint pk_operaciones_proyectotareas primary key,
...
create table operaciones.almacenes (
    almacenid int GENERATED ALWAYS AS IDENTITY constraint pk_operaciones_almacenes primary key,
...
create table operaciones.pedidosventa (
    pedidoid int GENERATED ALWAYS AS IDENTITY constraint pk_operaciones_pedidosventa primary key,
...
create table operaciones.pedidosventadetalle (
    detalledid int GENERATED ALWAYS AS IDENTITY constraint pk_operaciones_pedidosventadetalle primary key,
...
create table operaciones.ordenescompra (
    ordenid int GENERATED ALWAYS AS IDENTITY constraint pk_operaciones_ordenescompra primary key,
...
create table operaciones.ordenescompradetalle (
    detalledoc int GENERATED ALWAYS AS IDENTITY constraint pk_operaciones_ordenescompradetalle primary key,
...
create table operaciones.comprobantesfacturacion (
    comprobanteid int GENERATED ALWAYS AS IDENTITY constraint pk_operaciones_comprobantes primary key,
...
create table operaciones.guiasremision (
    guiaid int GENERATED ALWAYS AS IDENTITY constraint pk_operaciones_guias primary key,
...
create table operaciones.kardexmovimientos (
    movimientoid bigint GENERATED ALWAYS AS IDENTITY constraint pk_operaciones_kardex primary key,
...
-- ==========================================================================================
CREATE INDEX ix_pedidos_busqueda on operaciones.pedidosventa (numeropedido) include (estado, totalneto);
CREATE INDEX ix_kardex_reporte on operaciones.kardexmovimientos (productoid, almacenid, fechamovimiento);
...
CREATE OR REPLACE VIEW operaciones.vw_operaciones_dashboard_proyectos as
select 
    p.proyectoid,
    p.nombreproyecto,
    ...
    COALESCE(avg(t.porcentajeprogreso), 0.00) as progresopromedio,
...
CREATE OR REPLACE VIEW operaciones.vw_operaciones_bandeja_facturacion as
select 
    cf.comprobanteid,
    cf.tipocomprobante,
    cf.serie,
...
create table finanzas.impuestos (
    impuestoid int GENERATED ALWAYS AS IDENTITY constraint pk_finanzas_impuestos primary key,
...
create table finanzas.plancuentas (
    ...
    aceptaasiento BOOLEAN DEFAULT TRUE
);

create table finanzas.asientoscabecera (
    asientoid bigint GENERATED ALWAYS AS IDENTITY constraint pk_finanzas_asientos primary key,
...
create table finanzas.asientosdetalle (
    asientodetalleid bigint GENERATED ALWAYS AS IDENTITY constraint pk_finanzas_asientosdetalle primary key,
...
create table finanzas.cuentasbancarias (
    cuentabancariaid int GENERATED ALWAYS AS IDENTITY constraint pk_finanzas_cuentasbancarias primary key,
...
create table finanzas.movimientostesoreria (
    movimientotesoreriaid bigint GENERATED ALWAYS AS IDENTITY constraint pk_finanzas_tesoreria primary key,
...
create table finanzas.activosfijos (
    activoid int GENERATED ALWAYS AS IDENTITY constraint pk_finanzas_activos primary key,
    ...
    valornetolibros NUMERIC(18,4) GENERATED ALWAYS AS (valorinicial - depreciacionacumulada) STORED,
...
-- ==========================================================================================
CREATE INDEX ix_asientos_busquedacuenta on finanzas.asientosdetalle (cuentacodigo) include (debe, haber);
CREATE INDEX ix_tesoreria_flujocaja on finanzas.movimientostesoreria (cuentabancariaid, tipoflujo) include (monto, fechamovimiento);
...
CREATE OR REPLACE VIEW finanzas.vw_finanzas_libro_diario as
select 
    ac.asientoid,
    ac.numeroasiento,
    ac.fechaasiento,
...
CREATE OR REPLACE VIEW finanzas.vw_finanzas_resumen_bancos as
select 
    cb.cuentabancariaid,
    cb.banconombre,
    cb.numerocuenta,
    cb.moneda,
    cb.saldoactual,
    COALESCE(sum(case when mt.tipoflujo = 'ing' then mt.monto else 0 end), 0.0000) as totalingresos,
    COALESCE(sum(case when mt.tipoflujo = 'egr' then mt.monto else 0 end), 0.0000) as totalegresos
from finanzas.cuentasbancarias cb
left join finanzas.movimientostesoreria mt on cb.cuentabancariaid = mt.cuentabancariaid
where cb.estado = TRUE
...
CREATE OR REPLACE PROCEDURE finanzas.sp_finanzas_procesar_cobro_factura(
...
DECLARE
    v_moneda char(3);
    v_numerodoc varchar(50);
    v_clienteid int;
    v_asientoid bigint;
    v_numero_asiento_generado varchar(20);
BEGIN
    -- 1. obtener datos del comprobante a cobrar
    SELECT moneda, (serie || '-' || correlativo), clienteid
    INTO v_moneda, v_numerodoc, v_clienteid
    FROM operaciones.comprobantesfacturacion
    WHERE comprobanteid = p_comprobanteid;

    IF v_numerodoc IS NULL THEN
        RAISE EXCEPTION 'error: el comprobante especificado no existe.';
    END IF;
    ...
    VALUES (
        p_cuentabancariaid, 'ing', p_mediopago, p_monto, p_comprobanteid, null, p_glosa, CURRENT_TIMESTAMP
    );
    ...
    SET estadosunat = 'aceptada_sunat'
    WHERE comprobanteid = p_comprobanteid;
...
create table sunat.declaraciones_sire (
    sireid int GENERATED ALWAYS AS IDENTITY constraint pk_sunat_sire primary key,
...
create table sunat.control_car_sire (
    carid bigint GENERATED ALWAYS AS IDENTITY constraint pk_sunat_car primary key,
...
create table sunat.cierres_ple (
    pleid int GENERATED ALWAYS AS IDENTITY constraint pk_sunat_ple primary key,
...
create table seguridad.usuario_tokens (
    tokenid bigint GENERATED ALWAYS AS IDENTITY constraint pk_seguridad_usuario_tokens primary key,
...
create table seguridad.usuario_sesiones (
    sesionid bigint GENERATED ALWAYS AS IDENTITY constraint pk_seguridad_usuario_sesiones primary key,
    usuarioid int not null, -- relación lógica con la tabla de usuarios
    ...
    es_activa BOOLEAN not null DEFAULT TRUE -- control rápido para revocación remota de sesiones de la ui
);
...
create table seguridad.usuario_intentos_login (
    intentoid bigint GENERATED ALWAYS AS IDENTITY constraint pk_seguridad_usuario_intentos_login primary key,
    ...
    exito BOOLEAN not null, -- 1 = ingreso correcto, 0 = fallo
...
create table seguridad.usuario_historial_passwords (
    historialid bigint GENERATED ALWAYS AS IDENTITY constraint pk_seguridad_usuario_historial_passwords primary key,
    usuarioid int not null, -- relación lógica con la tabla de usuarios
...
create table seguridad.usuario_mfa (
    mfaid int GENERATED ALWAYS AS IDENTITY constraint pk_seguridad_usuario_mfa primary key,
    ...
    es_activo BOOLEAN not null DEFAULT FALSE, -- interruptor general de obligatoriedad en el login
...
CREATE INDEX ix_seguridad_usuario_tokens_busqueda 
...
CREATE INDEX ix_seguridad_usuario_sesiones_usuario_activa 
...
CREATE INDEX ix_seguridad_usuario_intentos_ip_recientes 
...
CREATE INDEX ix_seguridad_usuario_historial_antiguos 
...
-- MÓDULO EXCLUSIVO: RECURSOS HUMANOS (RECURSOS)
-- =============================================================================
CREATE SCHEMA IF NOT EXISTS rrhh_recursos;
...
CREATE TABLE rrhh_recursos.centros_costos (
    centrocostoid INT GENERATED ALWAYS AS IDENTITY,
    codigo VARCHAR(10) NOT NULL,
    nombre VARCHAR(100) NOT NULL,
    descripcion VARCHAR(250) NULL,
    responsable VARCHAR(150) NULL, 
    estaactivo BOOLEAN NOT NULL CONSTRAINT df_centros_costos_estaactivo DEFAULT TRUE, -- Declaración en línea correcta
...
CREATE TABLE rrhh_recursos.feriados (
    feriadoid INT GENERATED ALWAYS AS IDENTITY,
    fecha DATE NOT NULL,
    descripcion VARCHAR(150) NOT NULL,
    estaactivo BOOLEAN NOT NULL CONSTRAINT df_feriados_estaactivo DEFAULT TRUE,
...
CREATE TABLE rrhh_recursos.usuarios_nomina (
    usuarionominaid INT GENERATED ALWAYS AS IDENTITY,
    usuario VARCHAR(50) NOT NULL,
    nombrecompleto VARCHAR(150) NOT NULL,
    rol VARCHAR(50) NOT NULL, 
    correo VARCHAR(100) NOT NULL,
    estaactivo BOOLEAN NOT NULL CONSTRAINT df_usuarios_nomina_estaactivo DEFAULT TRUE,
...
CREATE TABLE rrhh_recursos.empleados (
    ...
    estaactivo BOOLEAN NOT NULL CONSTRAINT df_empleados_estaactivo DEFAULT TRUE,
...
CREATE TABLE rrhh_recursos.contratos (
    contratoid INT GENERATED ALWAYS AS IDENTITY,
    empleadoid INT NOT NULL,
    tipocontrato VARCHAR(50) NOT NULL, 
    fechainicio DATE NOT NULL,
    fechafin DATE NULL, 
    sueldobase DECIMAL(12,2) NOT NULL,
    estaactivo BOOLEAN NOT NULL CONSTRAINT df_contratos_estaactivo DEFAULT TRUE,
...
CREATE TABLE rrhh_recursos.regimenes_laborales (
    regimenlaboralid INT GENERATED ALWAYS AS IDENTITY,
    codigosunat VARCHAR(4) NOT NULL, 
    nombre VARCHAR(100) NOT NULL,    
    estaactivo BOOLEAN NOT NULL CONSTRAINT df_regimenes_laborales_estaactivo DEFAULT TRUE,
...
CREATE TABLE rrhh_recursos.administradoras_pensiones (
    afpid INT GENERATED ALWAYS AS IDENTITY,
    codigosunat VARCHAR(4) NOT NULL, 
    nombre VARCHAR(50) NOT NULL,
    tipo CHAR(3) NOT NULL,           
    estaactivo BOOLEAN NOT NULL CONSTRAINT df_administradoras_pensiones_estaactivo DEFAULT TRUE,
...
CREATE TABLE rrhh_recursos.derechohabientes (
    derechohabienteid INT GENERATED ALWAYS AS IDENTITY,
    empleadoid INT NOT NULL,
...
CREATE TABLE rrhh_recursos.turnos (
    turnoid INT GENERATED ALWAYS AS IDENTITY,
    nombre VARCHAR(50) NOT NULL, 
    ...
    estaactivo BOOLEAN NOT NULL CONSTRAINT df_turnos_estaactivo DEFAULT TRUE,
    CONSTRAINT pk_turnos PRIMARY KEY (turnoid)
);

CREATE TABLE rrhh_recursos.marcaciones_biometricos (
    marcacionid BIGINT GENERATED ALWAYS AS IDENTITY,
    empleadoid INT NOT NULL,
...
CREATE TABLE rrhh_recursos.asistencias_diarias (
    asistenciadiariaid BIGINT GENERATED ALWAYS AS IDENTITY,
    empleadoid INT NOT NULL,
    fecha DATE NOT NULL,
...
CREATE TABLE rrhh_recursos.tipos_licencias (
    tipolicenciaid INT GENERATED ALWAYS AS IDENTITY,
    codigosunat VARCHAR(4) NOT NULL, 
    descripcion VARCHAR(150) NOT NULL,
    congocehaber BOOLEAN NOT NULL,
    essubsidiado BOOLEAN NOT NULL,      
    estaactivo BOOLEAN NOT NULL CONSTRAINT df_tipos_licencias_estaactivo DEFAULT TRUE,
...
CREATE TABLE rrhh_recursos.solicitudes_licencias (
    solicitudlicenciaid INT GENERATED ALWAYS AS IDENTITY,
    empleadoid INT NOT NULL,
...
CREATE TABLE rrhh_recursos.periodos_vacacionales (
    periodovacacionalid INT GENERATED ALWAYS AS IDENTITY,
    empleadoid INT NOT NULL,
    ...
    estaabierto BOOLEAN NOT NULL CONSTRAINT df_periodos_vac_estaabierto DEFAULT TRUE,        
...
CREATE TABLE rrhh_recursos.programacion_vacaciones (
    programacionvacacionid INT GENERATED ALWAYS AS IDENTITY,
...
-- =============================================================================
CREATE SCHEMA IF NOT EXISTS rrhh_nomina;
...
CREATE TABLE rrhh_nomina.tasas_afp (
    tasasafpid INT GENERATED ALWAYS AS IDENTITY,
    afpid INT NOT NULL,
    anio INT NOT NULL,
    mes INT NOT NULL,
...
CREATE TABLE rrhh_nomina.conceptos (
    conceptoid INT GENERATED ALWAYS AS IDENTITY,
    codigosunat VARCHAR(4) NOT NULL,          
    nombre VARCHAR(120) NOT NULL,
    abreviatura VARCHAR(15) NOT NULL,
    tipoconcepto VARCHAR(30) NOT NULL,        
    esfijo BOOLEAN NOT NULL CONSTRAINT df_conceptos_esfijo DEFAULT FALSE,                      
    estaactivo BOOLEAN NOT NULL CONSTRAINT df_conceptos_estaactivo DEFAULT TRUE,
...
CREATE TABLE rrhh_nomina.conceptos_empleados_fijos (
    conceptoempleadofid INT GENERATED ALWAYS AS IDENTITY,
    empleadoid INT NOT NULL,
    conceptoid INT NOT NULL,
    montofijo DECIMAL(12,2) NOT NULL,
    explicacion VARCHAR(250) NULL,
    estaactivo BOOLEAN NOT NULL CONSTRAINT df_conceptos_emp_fijos_activo DEFAULT TRUE,
...
CREATE TABLE rrhh_nomina.periodos_planillas (
    periodoplanillaid INT GENERATED ALWAYS AS IDENTITY,
    anio INT NOT NULL,
    mes INT NOT NULL,
...
CREATE TABLE rrhh_nomina.planillas_cabeceras (
    planillacabeceraid INT GENERATED ALWAYS AS IDENTITY,
...
CREATE TABLE rrhh_nomina.planillas_detalles (
    planilladetalleid BIGINT GENERATED ALWAYS AS IDENTITY,
...
CREATE TABLE rrhh_nomina.planillas_conceptos_detalles (
    planillaconceptodetalleid BIGINT GENERATED ALWAYS AS IDENTITY,
...
CREATE TABLE rrhh_nomina.rentas_quinta_acumuladas (
    rentaquintaid INT GENERATED ALWAYS AS IDENTITY,
    empleadoid INT NOT NULL,
    anio INT NOT NULL,
...
CREATE TABLE sistema.parametros (
    parametroid INT GENERATED ALWAYS AS IDENTITY,
    clave VARCHAR(50) NOT NULL,
    valor TEXT NOT NULL,
    descripcion VARCHAR(250) NULL,
    categoria VARCHAR(50) NOT NULL,             
    fechamodificacion TIMESTAMP NOT NULL CONSTRAINT df_parametros_fechamodif DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE sistema.sesiones_usuarios (
    sesionid BIGINT GENERATED ALWAYS AS IDENTITY,
    usuario VARCHAR(50) NOT NULL,               
    fechaingreso TIMESTAMP NOT NULL CONSTRAINT df_sesiones_fechaingreso DEFAULT CURRENT_TIMESTAMP,
    fechasalida TIMESTAMP NULL,
    direccionip VARCHAR(45) NOT NULL,           
    dispositivo VARCHAR(250) NULL,              
    tokenacceso VARCHAR(250) NULL,              
    estasesionactiva BOOLEAN NOT NULL CONSTRAINT df_sesiones_activa DEFAULT TRUE,
...
CREATE TABLE sistema.logs_auditoria_datos (
    ...
    fecharegistro TIMESTAMP NOT NULL CONSTRAINT df_logs_auditoria_fecha DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE sistema.reportes_config (
    ...
    estaactivo BOOLEAN NOT NULL CONSTRAINT df_reportes_config_activo DEFAULT TRUE,
...
CREATE TABLE sistema.historial_descargas_reportes (
    descargareporteid BIGINT GENERATED ALWAYS AS IDENTITY,
    reporteid INT NOT NULL,
    usuario VARCHAR(50) NOT NULL,               
    fechageneracion TIMESTAMP NOT NULL CONSTRAINT df_historial_rep_fecha DEFAULT CURRENT_TIMESTAMP,
    parametrosusados TEXT NULL,         
